	...
	for _, point := range points {
		...
		case 'R':
			for i := int64(0); i < steps; i++ {
				items = append(items, fmt.Sprintf("%d_%d", startX+i, startY))
			}
			startX += steps
		case 'L':
			for i := int64(0); i < steps; i++ {
		...
		case 'U':
			for i := int64(0); i < steps; i++ {
				items = append(items, fmt.Sprintf("%d_%d", startX, startY+i))
			}
			startY += steps
		case 'D':
			for i := int64(0); i < steps; i++ {
...
func main() {
	one := getPoints(parseLine(line_one))
	two := getPoints(parseLine(line_two))
	smallest := int64(100000000)

	for _, i := range one {
		for _, j := range two {
			if i == j {
				it := strings.Split(i, "_")
				a, _ := strconv.ParseInt(it[0], 10, 64)
				b, _ := strconv.ParseInt(it[1], 10, 64)
				if a < 0 {
					a *= -1
				}
				if b < 0 {
	...
	log.Printf("smallest: %d", smallest)
}
